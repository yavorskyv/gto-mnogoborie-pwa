              ...
              <tr>
                ...
                <td>
                  <div style="display:flex;flex-direction:column;gap:3px">
                    ${(ev.hasOnline || /онлайн|online/i.test(ev.format || '') || /герой|народн/i.test(ev.title || '')) ? `
                      <span class="admin-badge info" style="background:rgba(56,189,248,0.15);color:#38bdf8;border:1px solid rgba(56,189,248,0.3)">⚡ Онлайн (Герой ГТО)</span>
                    ` : ''}
                    ${ev.hasOnsite !== false ? `
                      <span class="admin-badge neutral" style="font-size:10px">${esc(ev.format || 'Очный старт')}</span>
                    ` : ''}
                  </div>
                </td>
                <td>
...
function openEventEditorModal(ev) {
  ...
  modalContainer.innerHTML = `
    <div class="admin-modal-backdrop" id="admin-event-modal">
      <div class="admin-modal-dialog">
        ...
        <div class="admin-form-row">
          ...
          <div class="admin-form-group">
            ...
            <select class="admin-form-select" id="aef-format">
              <option value="Игры ГТО" ${ev?.format === 'Игры ГТО' ? 'selected' : ''}>Игры ГТО (Офлайн + Онлайн)</option>
              <option value="Кубок" ${ev?.format === 'Кубок' ? 'selected' : ''}>Кубок Федерации</option>
              <option value="Чемпионат" ${ev?.format === 'Чемпионат' ? 'selected' : ''}>Чемпионат России</option>
              <option value="Онлайн-отбор" ${ev?.format === 'Онлайн-отбор' ? 'selected' : ''}>Всероссийский онлайн-отбор</option>
            </select>
          </div>
        </div>
        ...
        <!-- Опции турнира: Иду на рекорд и Герой ГТО -->
        <div class="admin-form-group">
          <label style="font-weight:700;color:#38bdf8;font-size:12px;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:6px;display:block">
            Опции форматов участия в турнире:
          </label>
          <div style="display:flex;flex-direction:column;gap:8px;background:rgba(255,255,255,0.03);padding:12px;border-radius:10px;border:1px solid rgba(255,255,255,0.08)">
            <label class="admin-doc-check-item">
              <input type="checkbox" id="aef-opt-onsite" ${ev?.hasOnsite !== false ? 'checked' : ''}>
              <span>🏷️ <b>Очный турнир («Иду на рекорд»)</b> — мандатная комиссия, QR-пропуск, очный протокол</span>
            </label>
            <label class="admin-doc-check-item">
              <input type="checkbox" id="aef-opt-online" ${ev?.hasOnline !== false ? 'checked' : ''}>
              <span>⚡ <b>Онлайн-отбор («Герой ГТО»)</b> — видеофиксация комплекса WOD из 3 обязательных упражнений</span>
            </label>
          </div>
        </div>

        <div class="admin-form-group" id="aef-wod-config-wrap" style="display:${ev?.hasOnline !== false ? 'block' : 'none'}">
          <label style="font-size:12px;color:#cbd5e1;margin-bottom:4px;display:block">Дисциплины соревновательного комплекса (3 упражнения):</label>
  ...
  document.getElementById('admin-save-event-btn').onclick = async () => {
    ...
    const hasOnsite = document.getElementById('aef-opt-onsite')?.checked ?? true;
    const hasOnline = document.getElementById('aef-opt-online')?.checked ?? true;
    const wodExercises = [
    ...
    const updated = {
      ...
      format: document.getElementById('aef-format')?.value,
    ...
    try {
      await createOrUpdateMasterEvent({
        ...
        city: updated.city,
        format: hasOnline ? 'ONLINE' : (updated.format || 'OFFLINE'),
        hasOnsite,
        hasOnline,
        wodExercises
      });
    ...
    store.auditLog.unshift({
      time: new Date().toLocaleTimeString('ru-RU'),
      text: `${isNew ? 'Создание' : 'Редактирование'} мероприятия: ${title} (Иду на рекорд: ${hasOnsite ? 'Да' : 'Нет'}, Герой ГТО: ${hasOnline ? 'Да' : 'Нет'})`,
      user: 'admin@gto.com.ru'
    });
    saveAdminStore();
    modalContainer.innerHTML = '';
