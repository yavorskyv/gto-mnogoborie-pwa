...
class MasterDataService {
  ...
  createOrUpdateEvent(raw = {}) {
    ...
    const hasOnline = raw.hasOnline === true || raw.format === 'ONLINE';
    const hasRecordTracking = raw.hasRecordTracking === true;
    ...
    const event = {
      ...
      format: hasOnline ? 'ONLINE' : (raw.format || 'Очные соревнования'),
      status: raw.status || 'UPCOMING',
      hasOnsite: raw.hasOnsite !== false,
      hasOnline: hasOnline,
      hasRecordTracking: hasRecordTracking,
      wodExercises: hasOnline && Array.isArray(raw.wodExercises) && raw.wodExercises.length ? raw.wodExercises : [],
